...
**目标**: 逐个完成波次内的任务：思考→编码→验证→提交；并在 **Wave 边界内建** 整波静态审查，避免 Agent「写顺了」漏掉 `code-reviewer`。

> [!IMPORTANT]
> **严格按以下流程执行每个任务，不跳步。**

### Wave 内建结构（Wave = 任务循环 + 合拢审查）

把 **一次 Wave** 视为 **两段式流水线**（都发生在 Step 3 内，**未完成不得进入 Step 4**）：


| 阶段                                 | 内容                                                                             | 完成判据                                                                    |
| ---------------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------- |
| **A. 逐任务循环**                       | 对本波每个任务依次执行 **§3.1 → §3.6**                                                    | 每个任务的验收、验证证据与 commit 已落地；`05_TASKS.md` 已按规则回写                           |
| **B. Wave 合拢（内建 `code-reviewer`）** | 在 **本波最后一个任务** 的 **§3.4 自动验证全部通过** 之后，执行 **§3.4.5** 一次，覆盖 **本波已合并的实现变更**（整波汇总） | **§3.4.5 已执行并产出审查结果**（或已按 `code-reviewer` skill 记录**合法豁免**与补跑窗口）；不得无声跳过 |


> **说明**：阶段 B 不是「额外附录」，而是 **Wave 的固定组成部分**——与你在 `AGENTS.md` 的 `Wave` 块里规划的波次一一对应；波内中间任务仍 **不重复**跑 §3.4.5，但 **Wave 末那一次不可省**。与 **Step 4 §4.0** 合拢门禁互为校验：未满足不得宣告 Wave 结算完成。
...
- 若全部自动验证类型通过 → **§3.4.6**：若 skill 触发则按下方 **3.4.6** 执行；未触发则一行原因 → **3.5 → 3.6**。**§3.4.5 整波合拢**不在此「按任务二选一」——唯一挂载与门禁见 **Step 3「Wave 内建结构」阶段 B**、本节 **§3.4.5** 与 **Step 4 §4.0**。

### 3.4.5 / 3.4.6 频控（补充）

> **3.4.5**：Wave 末整波 `**code-reviewer`** 的**结构位置**已写在 Step 3「Wave 内建结构」**阶段 B**；**豁免、强度、证据**一律以 `**code-reviewer`** skill 为准。此处仅保留 **不与上文重复** 的补充：
>
> - **兜底**：连续约 **2～3 个 Wave** 漏做 Wave 末整波 **3.4.5** → 须在下一 Wave **Step 1** 前或上一 Wave **Step 4** 末 **补跑**（见 skill）。
> - **例外**（可在**本任务后**立刻加跑一轮 3.4.5）：用户/任务显式要求 **逐任务加强审**、**公共契约高风险**、`**/forge` AUTO 长会话**加密审 — 仍须服从 skill。
>
> **3.4.6（`e2e-testing-guide`）与浏览器工具**（与 3.4.5 **独立**，仍可按任务触发）：
>
...
（何时跑、可否跳过：**以 Step 3「Wave 内建结构」阶段 B + Step 4 §4.0 为准**；本小节只规定 **怎么跑** `code-reviewer`。）
...
完整遵从 `**e2e-testing-guide`** skill（触发条件、guide-only、证据规则、**PRD 对照 / 打分 / 状态枚举**均以 skill 为准）。

验证以**用户旅程**为主线：步骤、断言、证据与 `PASS` / `PARTIAL_PASS` / `FAIL` / `BLOCKED` / `NOT_RUN` / `INCONCLUSIVE`（Maybe）及 **JourneyScore** 规则均以 skill 为准。
...
- **§3.4.5 已完成**：本 Wave 已在 **最后一个任务** 后执行整波 `**code-reviewer`**（见 Step 3 **「Wave 内建结构」阶段 B** 与 **§3.4.5**），审查产物可追溯；或
- **合法豁免已落盘**：按 `code-reviewer` skill 的跳过协议写明原因、责任人与 **补跑时间点**（例如下一 Wave Step 1 之前）。

**不满足 → 禁止**更新「Wave 已完成」类叙事、**禁止**进入 Step 4 后续小节（4.1 及之后）当作正常结算——应先回到 Step 3 补齐 §3.4.5。
...
- 每个任务的验收标准全部通过
- 每个任务的遵从性检查全部通过
- **3.4.5 / 3.4.6**：已与 **Step 3 阶段 B、§3.4.5–3.4.6、Step 4 §4.0** 对齐；未执行项须有 **skill 合规**原因或补跑计划（不得空白带过）
